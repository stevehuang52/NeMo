...
parser.add_argument("-m", "--mode", default="exist", help="mode for cleaning")
...
def filter_manifest(data, key="audio_filepath", target="laugh"):
    results = []
    durations = 0.0
    for item in tqdm(data):
        entry = item[key].lower()
...
def main():
    ...
    for manifest in manifests_list:
        ...
        if args.mode == "exist":
            ...
            data_cleaned, duration_cleaned = filter_manifest(data)
            out_file = Path(manifest).stem + "_filtered.json"
