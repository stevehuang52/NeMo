...
from nemo.collections.asr.modules import (
    ASRTransformerEncoder,
    ASRTransformerFeatureExtractor,
...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            encoder_layers: Number of encoder layers to aggregate.
        ...
        if encoder_layers is None:
            if layer_idx_list is None:
                raise ValueError("If `encoder_layers` is None, `layer_idx_list` must be provided.")
            else:
                encoder_layers = len(layer_idx_list)
        ...
        if self.mode == "weighted_sum" and self.weights is None:
            self.weights = nn.Parameter(torch.ones(encoder_layers) / encoder_layers)
...
class ASRTransformerMultiLayerFeaturePreprocessor(NeuralModule, Exportable, AccessMixin):
    def __init__(
        self,
        aggregator: nn.Module,
        preprocessor: AudioToMelSpectrogramPreprocessor,
        ...
        self.preprocessor = preprocessor
        self.spec_augmentation = spec_augment
        self.feature_extractor = ASRTransformerFeatureExtractor(
            encoder=encoder, aggregator=aggregator, layer_idx_list=layer_idx_list
        )
        self.freeze_encoder = freeze_encoder
        if freeze_encoder:
